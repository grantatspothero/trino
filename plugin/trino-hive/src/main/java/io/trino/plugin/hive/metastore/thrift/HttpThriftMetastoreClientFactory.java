        ...
        implements ThriftMetastoreClientFactory
{
            ...
            throws TTransportException
    {
        ...
        URI useUri;
        try {
            useUri = new URI(uri.getScheme(), uri.getUserInfo(), useAddress.getHost(), useAddress.getPort(), uri.getPath(), uri.getQuery(), uri.getFragment());
        }
        catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
        return new THttpClient(useUri.toString(), httpClientBuilder.build());
    }
}
