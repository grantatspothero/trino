...
@TestsEnvironment
public class EnvMultinodeWithLegacyPathsTrino415
        extends EnvironmentProvider
{
    ...
    @Inject
    public EnvMultinodeWithLegacyPathsTrino415(PortBinder portBinder, DockerFiles dockerFiles, StandardMultinode standardMultinode, JdkProvider jdkProvider)
    {
    ...
    {
        DockerFiles.ResourceProvider configDir = dockerFiles.getDockerFilesHostDirectory("conf/environment/multinode-with-legacy-paths-trino-415");

        builder.configureContainer(TESTS, EnvMultinodeWithLegacyPathsTrino415::exportAWSCredentials);
        ...
        builder.configureContainer(COORDINATOR, EnvMultinodeWithLegacyPathsTrino415::exportAWSCredentials);

        builder.configureContainer(WORKER, container -> container
                .withCopyFileToContainer(forHostPath(configDir.getPath("iceberg.properties")), CONTAINER_TRINO_ICEBERG_PROPERTIES)
                .withCopyFileToContainer(forHostPath(configDir.getPath("delta.properties")), CONTAINER_TRINO_ETC + "/catalog/delta.properties"));
        builder.configureContainer(WORKER, EnvMultinodeWithLegacyPathsTrino415::exportAWSCredentials);
