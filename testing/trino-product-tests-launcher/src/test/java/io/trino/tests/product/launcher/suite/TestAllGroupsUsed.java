...
public class TestAllGroupsUsed
{
    ...
    public void testAllTestGroupsUsedInSuites()
    {
        ...
        // Ignore groups that are correctly defined but excluded from suites only in Galaxy
        usedGroups = new HashSet<>(usedGroups);
        verify(usedGroups.add(HIVE_KERBEROS));
        verify(usedGroups.add(HIVE_FILE_HEADER)); // excluded together with HIVE_KERBEROS
        verify(usedGroups.add(KUDU)); // run only for kerberized Kudu, also skipped for Galaxy
