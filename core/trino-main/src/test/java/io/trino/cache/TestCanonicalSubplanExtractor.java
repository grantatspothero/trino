        ...
        extends BasePlanTest
{
    ...
    public void testAggregationWithMultipleGroupByColumnsAndPredicate()
    {
        ...
        CacheColumnId regionKeyGreaterThan10 = canonicalExpressionToColumnId(new ComparisonExpression(GREATER_THAN, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 10L)));
        CacheColumnId regionKeyMultiplyBy2 = canonicalExpressionToColumnId(new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 2L)));
        assertThat(nonAggregatedSubplan.getAssignments()).containsExactly(
                entry(NATIONKEY_ID, NATIONKEY_REF),
                entry(NAME_ID, NAME_REF),
                entry(regionKeyGreaterThan10, new ComparisonExpression(GREATER_THAN, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 10L))),
                entry(regionKeyMultiplyBy2, new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 2L))));
        ...
        assertThat(aggregatedSubplan.getOriginalSymbolMapping()).containsOnlyKeys(
                ...
                NAME_ID,
                REGIONKEY_ID,
                canonicalExpressionToColumnId(new ComparisonExpression(GREATER_THAN, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 10L))),
                canonicalExpressionToColumnId(new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 2L))),
        ...
        assertThat(aggregatedSubplan.getAssignments()).containsExactly(
                ...
                entry(canonicalExpressionToColumnId(new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 2L))), columnIdToSymbol(regionKeyMultiplyBy2).toSymbolReference()),
    ...
    public void testAggregationWithMultipleGroupByColumns()
    {
        ...
        CacheTableId tableId = new CacheTableId(tpchCatalogId + ":tiny:nation:0.01");
        CacheColumnId nationKeyPlusOne = canonicalExpressionToColumnId(new ArithmeticBinaryExpression(ADD, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 1L)));
        CanonicalSubplan nonAggregatedSubplan = subplans.get(0);
        assertThat(nonAggregatedSubplan.getKeyChain()).containsExactly(new ScanFilterProjectKey(tableId));
        assertThat(nonAggregatedSubplan.getGroupByColumns()).isEmpty();
        assertThat(nonAggregatedSubplan.getAssignments()).containsExactly(
                ...
                entry(nationKeyPlusOne, new ArithmeticBinaryExpression(ADD, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 1L))));
        assertThat(nonAggregatedSubplan.getTableScan()).isPresent();
        assertThat(nonAggregatedSubplan.getChildSubplan()).isEmpty();
        assertThat(nonAggregatedSubplan.getTableScan().get().getColumnHandles()).containsExactly(
                entry(NATIONKEY_ID, new TpchColumnHandle("nationkey", BIGINT)),
        ...
        assertThat(aggregatedSubplan.getOriginalSymbolMapping()).containsOnlyKeys(
                ...
                NAME_ID,
                REGIONKEY_ID,
                canonicalExpressionToColumnId(new ArithmeticBinaryExpression(ADD, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 1L))),
                canonicalExpressionToColumnId(sum));
        assertThat(aggregatedSubplan.getAssignments()).containsExactly(
                entry(NAME_ID, new SymbolReference("[name:varchar(25)]")),
    ...
    public void testNestedProjections()
    {
        ...
        Expression nationKeyMultiplyBy2 = new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("[nationkey:bigint]"), GenericLiteral.constant(BIGINT, 2L));
        Expression regionKeyPredicate = new ComparisonExpression(GREATER_THAN, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 10L));
        CacheTableId tableId = new CacheTableId(tpchCatalogId + ":tiny:nation:0.01");
        ...
        Expression nationKeyPredicate = new ComparisonExpression(GREATER_THAN, new ArithmeticBinaryExpression(ADD,nationKeyMultiplyBy2Reference ,nationKeyMultiplyBy2Reference), GenericLiteral.constant(BIGINT, 10L));
    ...
    public void testTopNWithExpressionInOrderByColumn()
    {
        ...
        CanonicalSubplan scanSubplan = subplans.get(0);
        CacheColumnId regionKeyAdded5 = canonicalExpressionToColumnId(new ArithmeticBinaryExpression(ADD, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 5L)));
        assertThat(scanSubplan.getAssignments()).containsExactly(
                entry(NATIONKEY_ID, NATIONKEY_REF),
                entry(regionKeyAdded5, new ArithmeticBinaryExpression(ADD, new SymbolReference("[regionkey:bigint]"), GenericLiteral.constant(BIGINT, 5L))));
        CanonicalSubplan topNSubplan = subplans.get(1);
        assertThat(topNSubplan.getKey()).isInstanceOf(TopNKey.class);
        TopNKey key = (TopNKey) topNSubplan.getKey();
    ...
    public void testExtractCanonicalScanAndProject()
    {
        ...
        assertThat(subplan.getOriginalSymbolMapping()).containsExactly(
                ...
                entry(canonicalExpressionToColumnId(new ArithmeticBinaryExpression(ADD, new SymbolReference("[cache_column1]"), GenericLiteral.constant(BIGINT, 1L))), new Symbol("projection1")));
        assertThat(subplan.getAssignments()).containsExactly(
                entry(canonicalExpressionToColumnId(new ArithmeticBinaryExpression(ADD, new SymbolReference("[cache_column1]"), GenericLiteral.constant(BIGINT, 1L))), new ArithmeticBinaryExpression(ADD, new SymbolReference("[cache_column1]"), GenericLiteral.constant(BIGINT, 1L))),
    ...
    public void testExtractCanonicalFilterAndProject()
    {
        ...
        assertThat(predicate).isEqualTo(new ComparisonExpression(GREATER_THAN, new ArithmeticBinaryExpression(ADD, new SymbolReference("[cache_column1]"), new SymbolReference("[cache_column2]")), GenericLiteral.constant(BIGINT, 0L)));
    ...
    public void testProjectionWithDuplicatedExpressions()
    {
        assertThatCanonicalSubplanIsForTableScan(new ProjectNode(
                ...
                Assignments.of(
                        new Symbol("alias1"),
                        new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("symbol1"), GenericLiteral.constant(BIGINT, 2L)),
                        new Symbol("alias2"),
                        new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("symbol1"), GenericLiteral.constant(BIGINT, 2L)))));
    }
    ...
    private ProjectNode createScanAndProjectNode()
    {
        return new ProjectNode(
                ...
                Assignments.of(
                        ...
                        new ArithmeticBinaryExpression(ADD, new SymbolReference("symbol1"), GenericLiteral.constant(BIGINT, 1L)),
                        new Symbol("symbol2"),
    ...
    private FilterNode createFilterNode()
    {
        ...
        return new FilterNode(
                ...
                createTableScan(),
                and(
                        new ComparisonExpression(GREATER_THAN, new ArithmeticBinaryExpression(ADD, new SymbolReference("symbol1"), new SymbolReference("symbol2")), GenericLiteral.constant(BIGINT, 0L)),
