...
public class SqlTaskExecution
{
            ...
            Executor notificationExecutor)
    {
        ...
        try (SetThreadName ignored = new SetThreadName("Task-%s", taskId)) {
            List<SplitDriverFactory> driverFactories = localExecutionPlan.getDriverFactories();
            ...
            for (SplitDriverFactory driverFactory : driverFactories) {
            ...
            if (taskStateMachine.getState().isTerminatingOrDone()) {
                taskHandle = null;
                driverFactories.forEach(SplitDriverFactory::noMoreDrivers);
            }
            else {
        ...
        taskStateMachine.addStateChangeListener(state -> {
            if (state.isTerminatingOrDone()) {
                if (!taskHandle.isDestroyed()) {
                    ...
                    for (SplitDriverFactory factory : driverFactories) {
    ...
    {
        private final SplitDriverFactory driverFactory;
        ...
        private DriverSplitRunnerFactory(SplitDriverFactory driverFactory, Tracer tracer, boolean partitioned)
        {
